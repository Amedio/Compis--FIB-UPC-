...
boolexp: compexp ((AND^ | OR^) compexp)*;

compexp: intexp ((LESSTHAN^ | MORETHAN^ | EQUALS^) intexp)*;

intexp: multdiv ((PLUS^ | MINUS^) multdiv)*;

multdiv: negexp ((MULT^ | DIV^) negexp)*;

negexp: (parbraexp | NOT^ negexp | MINUS^ negexp);
